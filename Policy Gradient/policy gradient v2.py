...
class Policy(nn.Module):
    ...
    def __init__(self):
        ...
        self.state_head = nn.Linear(128,1)   # 状态价值函数

        self.saved_log_probs = []
        self.rewards = []
        #self.total_reward = []  # 用以保存每一个序列的reward
    ...
    def forward(self, x):
        ...
        state_values = self.state_head(x)
...
def select_action(state):
    ...
    policy.saved_log_probs.append((m.log_prob(action),state_value))    # 取对数似然 logπ(s,a)
...
def finish_episode():
    ...
    for (log_prob,state_value), R in zip(policy.saved_log_probs, returns):
        advantage = R - state_value
        policy_loss.append(-log_prob * advantage)          # 损失函数为交叉熵
        value_loss.append(F.smooth_l1_loss(state_value,R))
    optimizer.zero_grad()
    policy_loss = torch.stack(policy_loss).sum()          # 求和
...
def main():
    ...
    for i_episode in range(1000):        # 采集（训练）最多1000个序列
        ...
        #print(state.shape)
        for t in range(1, 1000):     # 每个episode最多1000步
